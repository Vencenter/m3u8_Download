...
class bilibili_gui(QWidget):
    ...
    def setMemory(self):
        ...
        if str(self.down_address.text())!="":
            self.loadImage(data)
            if not os.path.exists(ffpmpegRoot+"/cache/history.part"):
                memory_list.append(str(self.down_address.text()))
                memory_list.append(str(self.save_address.text()))
                memory_list.append(str(self.file_name.text()))

                with open(ffpmpegRoot+"/cache/history.part","w") as file:
                ...
                with open(ffpmpegRoot+"/cache/history.part") as file:
                    ...
                    if str(self.down_address.text())=="":
                        memory_list.append(filelink)
                    else:
                        memory_list.append(str(self.down_address.text()))
                        
                    if str(self.save_address.text())=="":
                        memory_list.append(filepos)
                    else:
                        memory_list.append(str(self.save_address.text()))
    ...
    def openVideo(self):
        ...
        if os.path.exists(ffpmpegRoot+"/cache/history.part"):
            ...
            filepos=""
            filename=""
        if str(self.down_address.text())!="" and str(self.save_address.text())!="" and str(self.file_name.text())!="":
            path= str(self.save_address.text()).decode("utf-8")
            if os.path.isdir(path):
                os.startfile(path)
                
        elif str(self.down_address.text())!="" and str(self.save_address.text())!='' and str(self.file_name.text())=="":
            self.file_name.setText(filename)
        elif str(self.save_address.text())!='' and str(self.file_name.text())!="" and str(self.down_address.text())=="":                                   
            self.down_address.setText(filelink)
        elif str(self.file_name.text())!="" and str(self.down_address.text())!="" and  str(self.save_address.text())=='':
